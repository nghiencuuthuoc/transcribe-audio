...
parser.add_argument('-i', '--input', type=str, default="../mp3-test", help='Input folder containing audio files')
...
# === Load Whisper model ===
model = whisper.load_model("large")  # or "small", "medium", "large-v3"
...
def save_pdf(text, path):
    ...
    styles.add(ParagraphStyle(name='Justify', alignment=4, fontName='Arial', fontSize=12, leading=18))
...
for full_path in all_files:
    ...
    try:
        ...
        txt_path = os.path.join(os.path.dirname(full_path), base_name + ".txt")
        docx_path = os.path.join(os.path.dirname(full_path), base_name + ".docx")
        pdf_path = os.path.join(os.path.dirname(full_path), base_name + ".pdf")
